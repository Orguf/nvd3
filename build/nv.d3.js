/* nvd3 version 1.8.1 (https://github.com/novus/nvd3) 2015-06-26 */
(function(){
...
nv.models.pie2 = function() {
    ...
    function chart(selection) {
        ...
        selection.each(function(data) {
            ...
            var ae = slices.enter().append('g');
            ae.attr('class', 'nv-slice');
            ae.property('selected', false);

            ae.on('mouseover', function(d, i) {
                d3.select(this).classed('hover', true);
                if(growOnHover && !d3.select(this).property("selected")) {
                    d3.select(this).select("path").transition()
                        .duration(70)
                        .attr("d", arcsOver[i]);
                }
                dispatch.elementMouseover({
                    ...
                    index: i,
                    color: d3.select(this).style("fill"),
                    selected: d3.select(this).property("selected")
                });
            });
            ae.on('mouseout', function(d, i) {
                d3.select(this).classed('hover', false);
            ...
            ae.on('click', function(d, i) {
                if(keepSelected){
                    ...
                    else if (d3.select(this).property("selected")){
                        d3.select(this).select("path").transition()
                            .duration(70)
                            .attr("d", arcsOver[i]);
                    }
